...
class GoogleMapsAPIScraper:
    ...
    def _get_request(
        ...
        query = f"https://www.google.com/async/reviewSort?authuser=0&hl={hl}&yv=3&cs=1&async=feature_id:{feature_id},review_source:All%20reviews,sort_by:{sort_by_id},is_owner:false,filter_text:,associated_topic:{associated_topic},next_page_token:{token},_pms:s,_fmt:pc"
        # query = (
...
if __name__ == "__main__":
    try:
        with GoogleMapsAPIScraper() as scraper:
            response_text, response_soup, reviews_soup, review_count, next_token = scraper._format_response_text(bt.read_html("google.com"))
            rs = []
